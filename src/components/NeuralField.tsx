...
export function NeuralField({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      for (const p of particles) {
        ...
        ctx.beginPath();
        ctx.arc(p.x, p.y, r * (p.spark ? 10 : 6), 0, Math.PI * 2);
        ctx.fill();
      }

      // Links. O(N^2) cap with spatial skipping for perf.
      ctx.lineWidth = 0.6;
  ...
  return (
    <canvas
      ref={ref}
      className={`${fixed ? "fixed" : "absolute"} inset-0 h-full w-full pointer-events-none ${className}`}
      aria-hidden
    />
  );
}
